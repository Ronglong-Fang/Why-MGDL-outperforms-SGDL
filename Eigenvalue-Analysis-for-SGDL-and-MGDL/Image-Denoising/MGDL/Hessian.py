...
def compute_Hessian_four(opt, params, z1, z2, z3, z4, a1, a2, a3, a4, output, train_x, train_y):
    ...
    for s in range(opt.nx):
        for t in range(opt.nx):
            ...
            grad_b1W3 += np.dot(temp, temp5_a2) + temp5_temp1_gradz2 * e                                  
            ...
            if (s==0 and t!=0) or (s!=0 and t==0):
                if s==0:
                    i0 = s*opt.nx + t-1
                    e0 = output[:, i] - output[:, i0] - u1_i
                else:
                    i0 = (s-1)*opt.nx + t
                    e0 = output[:, i] - output[:, i0] - u2_i
                ...
                diff0_temp5_a2 = np.kron(temp5, a2_i) - np.kron(temp50, a5_i0)
            ...
            if s!=0 and t!=0:
                ...
                diff1_temp5_a2 = np.kron(temp5, a2_i) - np.kron(temp51, a5_i1)
