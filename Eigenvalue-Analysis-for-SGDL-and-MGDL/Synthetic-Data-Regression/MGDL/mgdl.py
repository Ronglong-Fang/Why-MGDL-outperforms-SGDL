...
def analysis(filepath):
    ...
    for grade in range(1, opt.grade+1):
        ...
        if grade==1:
            ...
            plt.plot(xs_record[1:], history['val_loss'][1:], color="tab:orange", label="Validation loss")
        else:
            plt.plot(xs_record[1:], history['train_loss'][1:], color="tab:blue")
            plt.plot(xs_record[1:], history['val_loss'][1:], color="tab:orange")
    ...
    for grade in range(1, opt.grade+1):
        ...
        if grade==1:
            ...
            plt.plot(xs_record[1:], history['train_loss'][1:], color="tab:blue")
            plt.plot(xs_record[1:], history['val_loss'][1:], color="tab:orange", linestyle="--")
    ...
    fig_filename = f'Fig/MultiGrade_FrequencyFunction1_Loss.png'
    ...
    for grade in range(1, opt.grade+1):
        ...
        plt.plot(data["train_X"].T, history["train_acc"].T)
        plt.xlabel("$x$", fontsize=20)
        plt.ylabel("$y$", fontsize=20)
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        plt.title(f"Multi-Grade: Grade {grade}", fontsize=20)
        fig_filename = f'Fig/MultiGrade_FrequencyFunction1_predGrade{grade}.png'
        plt.savefig(fig_filename, format='png', bbox_inches='tight')
        plt.show()       
    ...
    opt.eig:
        ...
        for grade in range(1, opt.grade+1):
            ...
            for j in range(nshow):
                ...
                if grade==1:
                    plt.plot(range(sIter, sIter+history['xs'][-1], opt.interval)[start:], np.array(Eig_dict_MIN['Index'+str(j)]).T, label=f"Index {j}")
                else:
                    plt.plot(range(sIter, sIter+history['xs'][-1], opt.interval)[start:], np.array(Eig_dict_MIN['Index'+str(j)]).T)
    
            for j in range(nshow):
    
                if grade==1:
                    plt.plot(range(sIter, sIter+history['xs'][-1], opt.interval)[start:], np.array(Eig_dict_MAX['Index'+str(j)]).T, linestyle="--", label=f"Index {j+nshow}")
                else:
                    plt.plot(range(sIter, sIter+history['xs'][-1], opt.interval)[start:], np.array(Eig_dict_MAX['Index'+str(j)]).T, linestyle="--")
        ...
        fig_filename = f'Fig/MultiGrade_FrequencyFunction1_Eig.png'
        plt.savefig(fig_filename, format='png', bbox_inches='tight')
        plt.show()
    ...
    # plt.yticks(fontsize=20)
    # plt.ticklabel_format(style='sci', axis='x', scilimits=(0, 0))
    # plt.title('Multi-Grade: Ten largest eigenvalues of $\mathbf{I} - \eta\mathbf{H}_f(\mathbf{W}^k)$', fontsize=20)
    # fig_filename = f'Fig/MultiGrade_FrequencyFunction1_TenLargest.png'
    # plt.savefig(fig_filename, format='png', bbox_inches='tight')
    # plt.show()
