...
def train_model(grade, data, scaleFactor, train_data, accumulation_img, u, opt):
    ...
    model_loss = jit(lambda params, u, inputs_x, output: 
                     .5 * np.sum((model_pred(params, inputs_x) - output) ** 2) +  
                     .5 * opt.beta * np.sum((tv_matrix_form(model_pred(params, inputs_x)) - u)**2) + opt.lambd * np.sum(np.abs(u))
                     )
    
    mseloss = jit(lambda params, inputs_x, output: np.sqrt(np.mean((model_pred(params, inputs_x) - output) ** 2) ))
...
# @profile
def analysis(filepath, LossPsnr_print, Fig_print):
    ...
    for grade in range(1, opt.grade+1):
        ...
        if Fig_print:
            ...
            fig_filename = f'Fig/SingleGrade_denoising{int(255*opt.noise_level)}_deepbutterfly.png'
    ...
    if LossPsnr_print:
        ...
        plt.xticks([0, 0.5e4, 1e4, 1.5e4, 2e4], fontsize=20)
        plt.tight_layout()
        fig_filename = f'Fig/SingleGrade_denoising{int(255*opt.noise_level)}_deepbutterfly_PSNR.png'
        ...
        plt.xticks([0, 0.5e4, 1e4, 1.5e4, 2e4], fontsize=20)
        plt.tight_layout()
        fig_filename = f'Fig/SingleGrade_denoising{int(255*opt.noise_level)}_deepbutterfly_Loss.png'
        plt.ticklabel_format(style='sci', axis='x', scilimits=(0, 0))
        plt.savefig(fig_filename, format='png', bbox_inches='tight', pad_inches=0.1)
        plt.show()
