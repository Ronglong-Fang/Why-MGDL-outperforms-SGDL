...
def data_setup(opt):
    ...
    fig_filename = f'Fig/MultiGrade_denoising_deepbutterfly_CleanImage.png'
    ...
    plt.imshow(np.uint8(255*train_y), cmap='gray')
    plt.axis('off')
    plt.title(f"Noisy image: $\sigma = {int(opt.noise_level*255)}$", fontsize=20)
    fig_filename = f'Fig/MultiGrade_denoising{int(opt.noise_level*255)}_deepbutterfly_NoisyImage.png'
    plt.savefig(fig_filename, format='png', bbox_inches='tight', pad_inches=0.1)
    plt.show()  
