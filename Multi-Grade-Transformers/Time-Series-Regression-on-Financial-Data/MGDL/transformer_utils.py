...
def build_windows(y, dates, T):
    ...
    for i in range(len(y) - T - 1):
        X.append(y[i:i+T])       
        Z.append(y[i+T])          
        d = pd.to_datetime(dates[i+T]).strftime("%Y-%m-%d") 
        D.append(d)
...
def train_model(opt, train_data, val_data, test_data, y_acc_train, y_acc_val,
                y_acc_test, y_train_org, y_val_org, y_test_org, normalize, grade):
    ...
    for epoch in tqdm(range(opt.epochs)):
        ...
        if (epoch+1) % opt.loss_record == 0:
            ...
            y_val_batch, _ = predict_in_batches(model, state.params, val_data[0], batch_size=opt.batch_size)
            val_mse = normalize * normalize * mse_loss(jnp.array(y_val_batch), jnp.array(val_data[1]))
    ...
    y_val, feature_val = predict_in_batches(model, state.params, val_data[0], batch_size=opt.batch_size)
    ...
    res_y_val = (y_val_org - y_acc_val)/normalize
...
def MGDLmodel(opt):
    ...
    X_train_org, y_train_org, X_val_org, y_val_org, X_test_org, y_test_org, idx_train, idx_val, idx_test = split_data(X, Z)

    mean = X_train_org.mean()
    std = X_train_org.std()
    ...
    X_train_org = (X_train_org - mean) / std
    X_val_org   = (X_val_org - mean) / std
    X_test_org  = (X_test_org - mean) / std
    
    y_train_org = (y_train_org - mean) / std
    y_val_org   = (y_val_org - mean) / std
    y_test_org  = (y_test_org - mean) / std

    y = (y - mean) / std
    print("n train:", len(X_train_org))
    print("n val:", len(X_val_org))
    print("n test:", len(X_test_org))

    feature_train = X_train_org
    feature_val = X_val_org
    ...
    y_acc_val = jnp.zeros_like(y_val_org)
    ...
    for grade in range(1, opt.grade+1):
        ...
        val_data = [feature_val, res_y_val]
        test_data = [feature_test, res_y_test]
        
        s_time = time.time() 
        history = train_model(opt, train_data, val_data, test_data, y_acc_train, y_acc_val, y_acc_test, y_train_org, y_val_org, y_test_org, normalize, grade)
        e_time = time.time()

        feature_train = history['feature_train']
        feature_val = history['feature_val']
        feature_test = history['feature_test']
        
        res_y_train = history['res_y_train']
        res_y_val = history['res_y_val']
        res_y_test = history['res_y_test']

        y_acc_train = history['y_acc_train']
        y_acc_val = history['y_acc_val']
    ...
    picklename = 'results/MGDL_grade%d_epoch%d_TrMse%.2e_VaMSE%.2e_TeMSE%.2e.pickle' %(
        opt.grade, opt.epochs, history['Train_MSE'][-1], history['Val_MSE'][-1], history['Test_MSE']
    )
...
def analysis(filepath, predict):
    ...
    ite = 0 
    plt.axvline(x=ite, color='gray', linestyle='--')
    for grade in range(1, opt.grade+1):
        ...
        print(f"grade: {grade}, Train MSE: {history['Train_MSE'][-1]}, Val MSE: {history['Val_MSE'][-1]}, Test MSE: {history['Test_MSE']}, time: {history['time']}, total time: {time}")

        if grade==1:
            plt.plot(epoch_step, history['Train_MSE'], color="tab:blue", label="TrMSE")
            plt.plot(epoch_step, history['Val_MSE'], color="tab:orange", label="VaMSE")
        else:
            plt.plot(epoch_step, history['Train_MSE'], color="tab:blue")
            plt.plot(epoch_step, history['Val_MSE'], color="tab:orange")
    ...
    print(y_true_log_return_afterT)

    print(f"len y_true_afterT: {len(y_true_afterT)}, len y_true_log_return_afterT: {len(y_true_log_return_afterT)}")
    
    for grade in range(1, opt.grade+1):
        
        history = SaveHistory['grade'+str(grade)]
        y_hat_train = np.array(history['y_acc_train'])*std + mean
        y_hat_val = np.array(history['y_acc_val'])*std + mean
        y_hat_test = np.array(history['y_acc_test'])*std + mean

        y_hat_train_val_sorted = np.empty(n_train + n_val)
        ...
        y_hat_train_val_test_sorted[idx_train] = y_hat_train
        y_hat_train_val_test_sorted[idx_val] = y_hat_val
        y_hat_train_val_test_sorted[idx_test] = y_hat_test
    ...
    if predict:
        ...
        with pd.ExcelWriter("MGDL_SPX_Predictions.xlsx") as writer:
