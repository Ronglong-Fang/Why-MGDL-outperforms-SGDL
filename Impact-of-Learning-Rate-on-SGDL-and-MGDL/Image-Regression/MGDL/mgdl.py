...
def analysis(filepath):
    ...
    fig_filename = f'Fig/MultiGrade_deepbutterfly_Loss.png'
    ...
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    fig_filename = f'Fig/MultiGrade_deepbutterfly_PSNR.png'
    plt.ticklabel_format(style='sci', axis='x', scilimits=(0, 0))
    plt.savefig(fig_filename, format='png', bbox_inches='tight', pad_inches=0.1)
    plt.show()    
    ...
    for grade in range(1, opt.grade+1):
        ...
        plt.yticks(fontsize=20)
        plt.title(f"Multi-Grade: Grade {grade}", fontsize=20)
        fig_filename = f'Fig/MultiGrade_deepbutterfly_predGrade{grade}.png'
